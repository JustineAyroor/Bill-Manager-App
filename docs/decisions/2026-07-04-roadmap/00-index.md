...
5. Deployment hardening (systemd, Tailscale, CI/CD, backups) - **done, implemented and verified against the real production VM; see [10-deployment-implementation.md](10-deployment-implementation.md)**. Production intentionally stays on `master` (pre-multi-plan-schema) for now - promoting `dev` to production is a separate, deliberate next step.
