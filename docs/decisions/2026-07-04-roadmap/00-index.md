...
10. [Deployment Implementation Notes](10-deployment-implementation.md) - executing the deployment hardening deferred below: `uv` + systemd replacing `tmux`, Tailscale for private access, GitHub Actions CI/CD with a tightly-scoped deploy key, a two-layer backup strategy (local rotating SQLite backups + a credential-free GCE disk snapshot schedule), the decision to keep production on `master` (deliberately behind `dev`) until a dedicated promotion step, and a later incident where CI/CD deploys were silently failing on every push (missing execute bit on `deploy/deploy.sh`) while manual deploys masked the bug.
